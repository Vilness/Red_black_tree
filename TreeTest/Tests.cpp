...
namespace TreeTest
{
	...
	public:
		...
		template<typename T> using ContainerTemplate = RBTree<T, Mypred<T>, Myal<T>>;
	...
	public:
		...
		//template<typename T> using ContainerTemplate = RBTree<T, Mypred, Myal>;
	...
	public:
		...
		TEST_METHOD(FindTests)
		{
			ContainerTemplate<std::string> T1{ "abc", "cde", "123", "AAAAAAAA" };
			
			auto it1 = std::find(T1.begin(), T1.end(), "123");
			Assert::IsTrue(it1 != T1.end());
			Assert::IsTrue(*it1 == "123");

			it1 = std::find(T1.begin(), T1.end(), "dca");
			Assert::IsTrue(it1 == T1.end());

			ContainerTemplate<int> T2{ 9, 2, 43, 4, 5, 20 };

			auto it2 = std::find(T2.begin(), T2.end(), 20);
			Assert::IsTrue(it2 != T2.end());
			Assert::IsTrue(*it2 == 20);

			it2 = std::find(T2.begin(), T2.end(), 30);
			Assert::IsTrue(it2 == T2.end());
		}

		TEST_METHOD(CountIfTests)
		{
			ContainerTemplate<int> T1{ -1, 0, 2, -7, 5, -5, -8, 3 };

			Assert::IsTrue(std::count_if(T1.begin(), T1.end(), [](int x) { return x > 10; }) == 0);
			Assert::IsTrue(std::count_if(T1.begin(), T1.end(), [](int x) { return x < 0; }) == 4);
			Assert::IsTrue(std::count_if(T1.begin(), T1.end(), [](int x) { return x > 0; }) == 3);
			Assert::IsTrue(std::count_if(T1.begin(), T1.end(), [](int x) { return x == 0; }) == 1);
		}
	};
}